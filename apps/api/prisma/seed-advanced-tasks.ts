...
б) Найдите все корни этого уравнения, принадлежащие промежутку $\\left[0; \\dfrac{3\\pi}{2}\\right]$.`,
    correctAnswer: 'а) π/6+2πk, 5π/6+2πk, π/2+2πn; б) π/6, π/2, 5π/6',
...
$$t = \\dfrac{1}{2} \\quad \\text{или} \\quad t = 1$$

Оба значения входят в область допустимых значений синуса. Возвращаемся к переменной $x$:

1) $\\sin x = \\dfrac{1}{2}$:

$$x = \\dfrac{\\pi}{6} + 2\\pi k \\quad \\text{или} \\quad x = \\dfrac{5\\pi}{6} + 2\\pi k, \\quad k \\in \\mathbb{Z}$$

2) $\\sin x = 1$:

$$x = \\dfrac{\\pi}{2} + 2\\pi n, \\quad n \\in \\mathbb{Z}$$

**б) Отбор корней на промежутке $\\left[0; \\dfrac{3\\pi}{2}\\right]$**

Проверяем каждую серию, подставляя значения $k$ и $n$:

- $x = \\dfrac{\\pi}{6} + 2\\pi k$: подходит $k = 0$, тогда $x = \\dfrac{\\pi}{6}$. При $k = 1$ значение уже больше $\\dfrac{3\\pi}{2}$.

- $x = \\dfrac{5\\pi}{6} + 2\\pi k$: подходит $k = 0$, тогда $x = \\dfrac{5\\pi}{6}$.

- $x = \\dfrac{\\pi}{2} + 2\\pi n$: подходит $n = 0$, тогда $x = \\dfrac{\\pi}{2}$.

**Ответ:** а) $\\dfrac{\\pi}{6} + 2\\pi k$, $\\dfrac{5\\pi}{6} + 2\\pi k$, $\\dfrac{\\pi}{2} + 2\\pi n$; б) $\\dfrac{\\pi}{6}$, $\\dfrac{\\pi}{2}$, $\\dfrac{5\\pi}{6}$`,
    difficulty: 3,
  },
...
Ежегодное уменьшение долга: $\\dfrac{S}{n} = \\dfrac{600\,000}{3} = 200\,000$ рублей
...
Производительность первого рабочего: $\\dfrac{1}{10}$ заказа в день.

Производительность обоих вместе: $\\dfrac{1}{6}$ заказа в день.
...
**Ответ:** $15$ дней`,
  ...
  },
  {
    ...
    statement: `На рисунке изображён график функции $y = ax^2 + bx + c$.

![График функции](/tasks/parabola-up.svg)

Определите знак коэффициента $a$.`,
    correctAnswer: 'a > 0',
    referenceSolution: `**Решение**
